		...
		methods: {
			...
			formatDate() {
				...
				if (this.type === 'date' || this.type === 'year-month' || this.type === 'year') {
					...
					formatDateArray = this.dateTime.map((item, index) => {
						if (index > 2) {
							return this[names[index - 3] + 's'][item] < 10 ? addZero(this[names[index - 3] + 's'][item]) : this[names[
								index - 3] + 's'][item];
						} else {
							return this[name1[index] + 's'][item] < 10 ? addZero(this[name1[index] + 's'][item]) : this[name1[index] + 's']
								[item];
						}
					})
